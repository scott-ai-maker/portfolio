    ...
    <section id="projects" className="py-20">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {SAMPLE.map((p) => (
            <article key={p.title} className="group panel border rounded-lg p-5 hover:shadow-glow-md transition-shadow">
              <h3 className="text-lg font-medium text-text">{p.title}</h3>
              <p className="mt-2 text-muted">{p.desc}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {p.tech.map((t) => (
                  <span key={t} className="text-xs px-2 py-1 panel text-text/90 rounded">{t}</span>
                ))}
              </div>
